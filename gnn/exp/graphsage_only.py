...
class NodePairDecoder(nn.Module):
    """Decoder that uses node-pair embeddings only, without edge_attr."""
...
class GraphSAGEOnlyLinkPredictor(nn.Module):
    """GraphSAGE encoder plus node-pair decoder without edge features."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Experiment 2: GraphSAGE-only baseline without 631-D edge features."
    )
    ...
    parser.add_argument("--run-root", type=Path, default=ROOT / "exp/runs/graphsage_only")
...
@torch.no_grad()
def evaluate(
    model: GraphSAGEOnlyLinkPredictor,
    graph: dict[str, Tensor | np.ndarray],
    split: str,
    loss_fn: nn.Module,
    threshold: float,
    ...
    logits = model.decode(z, graph[f"{split}_edge_label_index"])  # type: ignore[arg-type]
...
def main() -> None:
    ...
    print(
        f"Experiment 2 GraphSAGE-only {args.species}: "
        ...
        f"decoder_in={args.hidden_dim * 4} device={device}"
    )
    ...
    final_metrics = {
        ...
        "interpretation": "If AUC drops strongly versus edge-feature models, edge_attr is the dominant signal.",
    }
