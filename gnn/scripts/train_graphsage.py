...
from mti_graphsage import GraphSAGELinkPredictor


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train GraphSAGE encoder + edge-feature decoder for miRNA-target interaction prediction."
    )
    ...
    parser.add_argument("--run-root", type=Path, default=ROOT / "runs/graphsage")
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Probability threshold for accuracy/F1 reporting.",
    )
...
def load_graph_npz(path: Path, device: torch.device, directed: bool) -> dict[str, Tensor | np.ndarray]:
    if not path.exists():
        raise FileNotFoundError(f"Missing GraphSAGE input file: {path}")
    # The numeric tensors are plain arrays, while sample/node IDs in the local
    # preprocessing output may be stored as object string arrays.
    ...
    for split in ["train", "val", "test"]:
        tensors[f"{split}_edge_label_index"] = torch.from_numpy(
            data[f"{split}_edge_label_index"]
        ).long().to(device)
        tensors[f"{split}_edge_label"] = torch.from_numpy(data[f"{split}_edge_label"]).float().to(device)
        tensors[f"{split}_edge_attr"] = torch.from_numpy(data[f"{split}_edge_attr"]).float().to(device)
        tensors[f"{split}_sample_id"] = data[f"{split}_sample_id"]
    return tensors
...
def evaluate(
    ...
    labels = graph[f"{split}_edge_label"]  # type: ignore[assignment]
...
def train_one_epoch(
    ...
    loss = loss_fn(logits, graph["train_edge_label"])  # type: ignore[arg-type]
    loss.backward()
    optimizer.step()
...
def save_predictions(
    ...
    labels = graph[f"{split}_edge_label"].detach().cpu().numpy().astype(int)  # type: ignore[union-attr]
...
def main() -> None:
    ...
    graph = load_graph_npz(data_path, device=device, directed=args.directed_message_passing)
    ...
    train_labels = graph["train_edge_label"]  # type: ignore[assignment]
    ...
    print(
        ...
        f"train_candidates={graph['train_edge_label'].numel()} "
    ...
    (run_dir / "metrics.json").write_text(
        json.dumps(json_safe(final_metrics), indent=2),
        encoding="utf-8",
    )
